...
%
\draw (nor1.input 1) -- ++(left:5mm) node[left] (B1) {$A$};
\draw (nor1.input 2) -- ++(left:5mm) node[left] (C1) {$B$};
\draw (nor1.output) -- ++(right:3mm) node[right] (Y1) {$Y$};

\draw (and1.input 1) -- ++(left:3mm) node[left] (B2) {$A$};
\draw (and1.input 2) -- ++(left:3mm) node[left] (C2) {$B$};
